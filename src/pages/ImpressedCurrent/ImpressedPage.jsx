// /src/pages/impressed/ImpressedPage.jsx
...
export default class ImpressedPage extends React.Component {
  constructor(props) {
    ...
    const saved =
      typeof window !== "undefined"
        ? window.localStorage.getItem("impressed_current_calc")
        : null;
    ...
    this.state = {
      ...
      error: null,
      activeTab: "results",
      savedInputs: parsed?.inputs || null, // ✅ used by HeaderSaveBar
    };
    ...
    this.setState({ submitting: true, error: null }, () => {
      try {
        ...
        if (missing.length) {
          ...
          this.setState({
            submitting: false,
            error: `Please fill the required field: ${first}.`,
          });
          return;
        }
        // Numeric validation
        ...
        for (const [k, rule] of numFields) {
          ...
          if (rule === "> 0" && !(raw > 0)) {
            this.setState({ submitting: false, error: `${k} must be greater than 0.` });
            return;
          }
          if (rule === ">= 0" && !(raw >= 0)) {
            this.setState({ submitting: false, error: `${k} must be at least 0.` });
            return;
          }
          if (rule === ">= 1" && !(raw >= 1)) {
            this.setState({ submitting: false, error: `${k} must be at least 1.` });
            return;
          }
        }
        // Units conversion
        ...
        const inputs = {
          ...
          I_single_A: Number(
            fd.get("I_single_A") || ((fd.get("environment") || "soil") === "seawater" ? 50 : 8)
          ),
        ...
        this.setState({
          results,
          savedInputs: inputs, // ✅ for HeaderSaveBar
          activeTab: "results",
        });
      } catch (err) {
        this.setState({
  ...
  onResetAll = () => {
    try {
      window.localStorage.removeItem("impressed_current_calc");
    } catch {
      /* ignore */
    }
    this.setState({
  ...
  InfoCard = () => (
    <ModuleCard
      title="Impressed Current System"
      subtitle="I = A×Jd×f_c; V = I×R + (E_target − E_native)"
      actions={<ResetPill onClick={this.onResetAll} />}
    >
      <div className="text-sm text-gray-700 dark:text-gray-300">
        <p>
          Design outputs include current, system voltage, power and annual energy.
          Anode quantity by FeSiCr mass or MMO current rating.
        </p>
      </div>
    </ModuleCard>
  );
  ...
  render() {
    ...
    const headerActions = (
      <HeaderSaveBar
        moduleKey="impressed_current_calc"
        moduleLabel="Impressed Current"
        inputs={savedInputs}
        results={results}
        captureRef={this.captureRef}
        // explicit backend formula name
        formulaName="Impressed Current System Calculation"
        modulePath="/pages/impressed-current"
        // Nice name in server history
        buildName={({ inputs, project }) => {
          const env = inputs?.environment || "soil";
          const A = Number(inputs?.area_m2 ?? 0) || 0;
          const Jd = Number(inputs?.jd_mA_per_m2 ?? 0) || 0;
          const R = Number(inputs?.R_ohm ?? 0) || 0;
          return `${project?.name || "Default"} • ${env} • A=${A} m² • Jd=${Jd} mA/m² • R=${R} Ω`;
        }}
      />
    );
