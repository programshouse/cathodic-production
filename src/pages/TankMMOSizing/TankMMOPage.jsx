// /src/pages/tank-mmo/TankMMOPage.jsx
...
export default class TankMMOPage extends React.Component {
  constructor(props) {
    ...
    if (typeof window !== "undefined") {
      try {
        parsed = JSON.parse(window.localStorage.getItem("tank_mmo_calc") || "null");
      } catch { /* ignore */ }
    }
    this.state = {
      submitting: false,
  ...
  onSubmit = (raw) => {
    ...
    if (
      !(Number(diameter_value) > 0) ||
      !(Number(tank_length_value) > 0) ||
      !(Number(spacing_value) > 0) ||
      !(Number(num_bars) >= 0) ||
      !(Number(connection_length_value) >= 0) ||
      !(Number(Iconnector_max_value) > 0)
    ) {
    ...
    const core = computeTankMMO({
      diameter_m: D,
      tank_length_m: L,
      install_type,
      spacing_m: s,
      nrings_manual: Number(nrings_manual || 0),
      num_bars: Number(num_bars || 0),
      connection_length_m: connL,
      Itotal_A,
      Iconnector_max_A: Iconn_A,
    });
    ...
    try {
      window.localStorage.setItem("tank_mmo_calc", JSON.stringify({ inputs: raw, results }));
    } catch { /* ignore */ }
  ...
  downloadResultsCsv = () => {
    ...
    a.href = url;
    a.download = "tank-mmo-sizing-results.csv";
  ...
  render() {
    ...
    // ✅ Header Save Bar under the header (consistent with other pages)
    const headerActions = (
      <HeaderSaveBar
        moduleKey="tank_mmo_calc"
        moduleLabel="Tank MMO Anode Sizing"
        inputs={savedInputs}
        results={results}
        captureRef={this.captureRef}
        formulaName="Tank MMO Anode Sizing"
        modulePath="/pages/tank-mmo"
        buildName={({ inputs, project }) => {
          const D = inputs?.diameter_value ? `${inputs.diameter_value} ${inputs.diameter_unit || ""}`.trim() : "—";
          const L = inputs?.tank_length_value ? `${inputs.tank_length_value} ${inputs.tank_length_unit || ""}`.trim() : "—";
          const t = inputs?.install_type || "—";
          const s = inputs?.spacing_value ? `${inputs.spacing_value} ${inputs.spacing_unit || ""}`.trim() : "—";
          return `${project?.name || "Default"} • ${t} • D=${D} • L=${L} • s=${s}`;
        }}
      />
    );
    ...
    return (
      <CalculatorPanel
            ...
            <div className="relative overflow-hidden rounded-2xl">
              ...
              <div className="relative px-5 py-5 md:px-7 md:py-6">
                <h2 className="text-white text-xl md:text-2xl font-semibold tracking-tight">
                  Tank MMO Anode Sizing
                </h2>
                <p className="text-brand-50/90 text-sm md:text-base mt-1">
                  Compute circumference, ribbon lengths, Ti bar length, and number of feeders from geometry and spacing.
                </p>
              </div>
            </div>
          </div>
        ...
        right={
          // 📸 This pane is captured into the PDF by HeaderSaveBar
