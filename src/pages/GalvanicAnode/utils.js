...
export function computeGalvanic({ area_m2, jd_mA_per_m2, coating_factor, design_life_years, material, eta, anode_weight_kg }) {
  ...
  const fc = Number(coating_factor || 1);
...
export function lifeSeries({ area_m2, jd_mA_per_m2, coating_factor, years_max = 30, material, eta, anode_weight_kg }) {
  const points = [];
  for (let y = 0; y <= years_max; y += Math.max(1, Math.floor(years_max / 10))) {
    const { W_required } = computeGalvanic({ area_m2, jd_mA_per_m2, coating_factor, design_life_years: y, material, eta, anode_weight_kg });
    points.push({ year: y, weight: W_required });
  }
  if (points[points.length - 1]?.year !== years_max) {
    const { W_required } = computeGalvanic({ area_m2, jd_mA_per_m2, coating_factor, design_life_years: years_max, material, eta, anode_weight_kg });
