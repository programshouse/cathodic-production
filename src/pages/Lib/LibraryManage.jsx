...
export default function LibraryManage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (typeFilter !== "all") {
      list = list.filter((f) =>
        (f.mime || f.mimetype || "").includes(typeFilter)
      );
    }
  ...
  const handleDetails = async (id) => {
    ...
    try {
      ...
      toast.update(t, {
        render: err?.message || "Failed to load",
        type: "error",
        isLoading: false,
        autoClose: 2500,
      });
  ...
  return (
    <PageLayout title="Library (Manage) | CP">
      {/* Header (لو محتاج ترجع تستخدمه) */}
      {/* <PageHeader
        title="Library Management"
        subtitle="Browse, search, and manage all technical documents and files."
      /> */}
      ...
      <CardBox>
        <div className="flex items-center justify-between mb-3">
          <div className="text-sm font-semibold" style={{ color: CP_BLUE }}>
            All Files
          </div>
        </div>
              ...
              {filtered.map((f) => (
                ...
                >
                  <td className="py-2 pr-4">
                    ...
                    {f.description && (
                      <div className="text-[12px] text-gray-500 dark:text-gray-400 line-clamp-2">
                        {f.description}
                      </div>
                    )}
                  ...
                  <Td className="py-2 pr-4">
                    <div className="flex items-center gap-2">
                      ...
                      <Btn onClick={() => handleDetails(f.id)} size="xs">
                        Show
                      </Btn>
                      <Btn
                        variant="danger"
                        size="xs"
                        onClick={() =>
                          confirmDelete(() => deletelibrary(f.id))
                        }
                      >
                        Delete
                      </Btn>
    ...
    <div className="group rounded-2xl border bg-white border-gray-200 p-4 flex flex-col gap-3 shadow-sm hover:shadow-md transition">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {f.description && (
            <div className="text-[12px] text-gray-500 line-clamp-2 mt-0.5">
              {f.description}
            </div>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 mt-auto">
        ...
        <button
          onClick={onDelete}
          className="text-xs px-2 py-1 rounded-full border ml-auto"
        ...
        <div className="m-0 md:m-4 rounded-t-2xl md:rounded-2xl overflow-hidden border bg-white shadow-xl">
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2 text-sm">
              <MetaRow
                label="Title"
                value={item.title || item.filename || `File #${item.id}`}
              />

              <MetaRow
                label="Uploaded"
                value={formatDateTime(item.created_at || item.uploaded_at)}
              />
            ...
            {/* ممكن تضيف هنا مساحة Preview لو حبيت بعدين (PDF, Image, إلخ) */}
          </div>
        </div>
      </div>
    </div>
...
function Th({ children, className = "" }) {
  return (
    <th
      className={`py-3 px-4 text-xs font-semibold tracking-wide ${className}`}
    >
      {children}
    </th>
  );
