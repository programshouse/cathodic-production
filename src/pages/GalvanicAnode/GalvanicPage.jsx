  ...
  onSubmit = (e) => {
    this.setState({ submitting: true, error: null }, () => {
      try {
        ...
        const calc = computeGalvanic({ area_m2, jd_mA_per_m2, coating_factor, design_life_years, material, eta: etaNum, anode_weight_kg });
        const lifeSeriesData = lifeSeries({ area_m2, jd_mA_per_m2, coating_factor, years_max: Math.max(5, design_life_years || 30), material, eta: etaNum, anode_weight_kg });
        this.setState({
    ...
    >
      <div className="text-sm md:text-base text-gray-700 dark:text-gray-300">
        ...
        <ul className="list-disc pl-6">
          {MATERIALS.map((m) => (
            <li key={m.value}>{m.label}: {m.capacity_Ah_per_kg} Ah/kg • η≈{(m.eta_default*100).toFixed(0)}%</li>
          ))}
        </ul>
      </div>
    </ModuleCard>
