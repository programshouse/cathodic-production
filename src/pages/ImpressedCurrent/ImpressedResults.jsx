...
import {
  ResponsiveContainer, LineChart, XAxis, YAxis, Tooltip, CartesianGrid, Line, ReferenceLine
} from "recharts";
...
export default function ImpressedResults({ results }) {
  ...
  const {
    I_A, I_mA, V_system, P_W, E_annual_kWh, anode, inputs
  } = results;

  // ---- normalize series for FeSiCr chart ----
  ...
  const series = rawSeries.map((d) => ({
    ...
    // map any 'weight' key into weight_kg for the chart
    weight_kg: Number(
      d.weight_kg ?? d.weight ?? d.w ?? 0
    ),
  }));
  ...
  const allRows = [
    ...
    { Label: "Power", Value_W: Number(P_W)||0, Unit_W: "W", Value_kW: (Number(P_W)||0)/1000, Unit_kW: "kW" },
  ...
  const safeFix = (v, d=3) => Number.isFinite(Number(v)) ? Number(v).toFixed(d) : "—";
    ...
    <div className="space-y-4">
      ...
      <ModuleCard title="Electrical Summary" subtitle="Required current, voltage, power, and annual energy.">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <div className="font-semibold">{safeFix(I_A,3)} A ({safeFix(I_mA,0)} mA)</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="font-semibold">{safeFix(E_annual_kWh,1)} kWh/yr</div>
          </div>
        </div>
      </ModuleCard>
      ...
      {inputs?.anode_type === "FeSiCr" && anode && (
        <ModuleCard title="FeSiCr Anode Requirement" subtitle="Total mass and quantity vs design life.">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-4">
            <div><div className="text-gray-500">Required Mass</div><div className="font-semibold">{safeFix(anode.W_required,1)} kg</div></div>
            <div><div className="text-gray-500">Unit Mass</div><div className="font-semibold">{safeFix(anode.W_single,1)} kg</div></div>
            <div><div className="text-gray-500">Safety Factor</div><div className="font-semibold">{safeFix(inputs.safety_factor,2)}</div></div>
            <div><div className="text-gray-500">Quantity</div><div className="font-semibold">{Math.ceil(Number(anode.N||0))}</div></div>
          </div>
          <div className="h-64">
            <ResponsiveContainer>
              <LineChart data={series} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                ...
                <YAxis domain={[0, yMax]} label={{ value: "Required Weight (kg)", angle: -90, position: "insideLeft" }} />
      ...
      {inputs?.anode_type === "MMO" && anode && (
        <ModuleCard title="MMO Anode Requirement" subtitle="Quantity based on current rating per anode.">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><div className="text-gray-500">Current per Anode</div><div className="font-semibold">{safeFix(anode.I_single_A,3)} A</div></div>
            <div><div className="text-gray-500">Safety Factor</div><div className="font-semibold">{safeFix(inputs.safety_factor,2)}</div></div>
            <div><div className="text-gray-500">Total Current</div><div className="font-semibold">{safeFix(I_A,3)} A</div></div>
            <div><div className="text-gray-500">Quantity</div><div className="font-semibold">{Math.ceil(Number(anode.N||0))}</div></div>
          </div>
        </ModuleCard>
      )}
